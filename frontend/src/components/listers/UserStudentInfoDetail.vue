<template>
    <v-card outlined>
        <v-card-title>
            StudentInfo # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="StudentNo" v-model="item.studentNo" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/studentInfos/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
